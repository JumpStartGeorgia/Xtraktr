/*global  $, jQuery, debounce, replaceChild, normalize, page_wrapper*/
var group_ids = [], codebook, li_items, li_length;
...
$(document).ready(function (){
  ...
  $("input#filter", codebook).keyup(debounce(function () { run_search(); }, 500));
  ...
  $(document).on("click", ".tab-content .up", function () { // top button scroller
    page_wrapper.animate({ scrollTop: 0 }, 1500);
  });

  $("select.selectpicker-group", codebook).change(function () { // when select group, go to it
    var t = $(this),
      item = $(".group-item[data-id='" + t.val() + "']", codebook);

    page_wrapper.animate({ scrollTop: page_wrapper.scrollTop() + item.offset().top - 51 + (item.hasClass("subgroup") ? 0 : 20) }, 1500);
    t.selectpicker("val", ""); // reset select to default value
  });

  $(".question-link-groups .question-link-group", codebook).click(function () { // when click on group under question link, go to it
    var item = $(".group-item[data-id='" + $(this).data("id") + "']", codebook);
    page_wrapper.animate({ scrollTop: page_wrapper.scrollTop() + item.offset().top - 51 + (item.hasClass("subgroup") ? 0 : 20) }, 1500);
  });

  $(".view-all label", codebook).click(function (){
    var t = $(this),
      p = t.parent();
